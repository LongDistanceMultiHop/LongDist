...
class PolicyGradient(LFramework):
    ...
    def rollout(self, e_s, q, e_t, num_steps, visualize_action_probs=False):
        ...
        if self.model.startswith('point.fusion'):
            fn,fn_kg=self.fn,self.fn_kg
        # Initialization
        #equation (2) LSTM(0,[r_0,e_s])
        log_action_probs = []
        action_entropy = []
        ...
        for t in range(num_steps):
            ...
            if self.model.startswith('point.fusion'):
                db_outcomes, inv_offset, policy_entropy = pn.transit_fusion(
                    e, obs, kg,fn,fn_kg, use_action_space_bucketing=self.use_action_space_bucketing)
            else:
    ...
    def predict(self, mini_batch, verbose=False):
        ...
        if self.model.startswith('point.fusion') and self.using_embedding_test==True:
            fn,fn_kg=self.fn,self.fn_kg
            beam_search_output = search.beam_search_fusion(
                pn, e1, r, e2, kg, fn,fn_kg, self.num_rollout_steps, self.beam_size)
        else:
