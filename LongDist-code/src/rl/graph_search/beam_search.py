...
def beam_search_fusion(pn, e_s, q, e_t, kg, fn,fn_kg,num_steps, beam_size, return_path_components=False):
    """
    Beam search from source.
    ...
    for t in range(num_steps):
        ...
        obs = [e_s, q, e_t, t == (num_steps - 1), last_r, seen_nodes]
        # one step forward in search
        db_outcomes, _, _ = pn.transit_fusion(
            e, obs, kg, fn, fn_kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=True)
