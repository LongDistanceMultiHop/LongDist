...
class GraphSearchPolicy(nn.Module):
    def __init__(self, args):
        ...
        self.using_attention=args.using_attention
        self.long_short_term=args.long_short_term
    ...
    def transit_fusion(self, e, obs, kg,fn,fn_kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=False):
        ...
        def policy_nn_fun_fusion(e,X2, action_space,fn,fn_kg):
            ...
            for i in range(dim1):
                ...
                em_score_can.append(torch.squeeze(fn.forward_fact(e1_can, r_can, e2_can, fn_kg)))
        ...
        if use_action_space_bucketing:
            ...
            entropy_list = []
            references = []
            db_e,db_action_spaces, db_references = self.get_action_space_in_buckets_fusion(e, obs, kg)
            for e_b,action_space_b, reference_b in zip(db_e,db_action_spaces, db_references):
                X2_b = X2[reference_b, :]
                action_dist_b, entropy_b = policy_nn_fun_fusion(e_b,X2_b, action_space_b,fn,fn_kg)
                references.extend(reference_b)
                db_outcomes.append((action_space_b, action_dist_b))
                entropy_list.append(entropy_b)
            inv_offset = [i for i, _ in sorted(enumerate(references), key=lambda x: x[1])]
            ...
            action_space = self.get_action_space(e, obs, kg)
            action_dist, entropy = policy_nn_fun_fusion(e,X2, action_space,fn,fn_kg)
            db_outcomes = [(action_space, action_dist)]
            inv_offset = None
    ...
    def get_action_space_in_buckets_fusion(self, e, obs, kg, collapse_entities=False):
        """
        To compute the search operation in batch, we group the action spaces of different states
        (i.e. the set of outgoing edges of different nodes) into buckets based on their sizes to
        save the memory consumption of paddings.
        ...
        assert(len(e) == len(q))
        assert(len(e) == len(e_t))
        db_action_spaces, db_references,db_e = [], [],[]
        ...
        return db_e,db_action_spaces, db_references
    ...
    def define_modules(self):
        ...
        #self.W3 = nn.Linear(self.history_dim, 1)
        if self.relation_only_in_path:
